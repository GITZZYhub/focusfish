...
class ScrollablePositionedList extends StatefulWidget {
  ...
  const ScrollablePositionedList.builder({
    required final this.itemCount,
    required final this.itemBuilder,
    final Key? key,
    final this.itemScrollController,
    final ItemPositionsListener? itemPositionsListener,
    final this.initialScrollIndex = 0,
    final this.initialAlignment = 0,
    final this.scrollDirection = Axis.vertical,
    final this.reverse = false,
    final this.physics,
    final this.semanticChildCount,
    final this.padding,
    final this.addSemanticIndexes = true,
    final this.addAutomaticKeepAlives = true,
    final this.addRepaintBoundaries = true,
    final this.minCacheExtent,
  ...
  const ScrollablePositionedList.separated({
    required final this.itemCount,
    required final this.itemBuilder,
    required final this.separatorBuilder,
    final Key? key,
    final this.itemScrollController,
    final ItemPositionsListener? itemPositionsListener,
    final this.initialScrollIndex = 0,
...
class ItemScrollController {
  ...
  void jumpTo({required final int index, final double alignment = 0}) {
  ...
  Future<void> scrollTo({
    ...
    final List<double> opacityAnimationWeights = const [40, 20, 40],
  }) {
  ...
  void _attach(final _ScrollablePositionedListState scrollableListState) {
    assert(_scrollableListState == null);
...
class _ScrollablePositionedListState extends State<ScrollablePositionedList>
  ...
  @override
  void didUpdateWidget(final ScrollablePositionedList oldWidget) {
  ...
  Widget build(final BuildContext context) => LayoutBuilder(
        builder: (final context, final constraints) {
          ...
          return GestureDetector(
            onPanDown: (final _) => _stopScroll(canceled: true),
                ...
                PostMountCallback(
                  ...
                  child: FadeTransition(
                    ...
                    child: NotificationListener<ScrollNotification>(
                      onNotification: (final notification) => _isTransitioning,
  ...
  double _cacheExtent(final BoxConstraints constraints) => max(
        constraints.maxHeight * _screenScrollCount,
        widget.minCacheExtent ?? 0,
      );

  void _jumpTo({required final int index, required final double alignment}) {
    _stopScroll(canceled: true);
    setState(() {
      primary.scrollController.jumpTo(0);
      primary
        ..target = min(index, widget.itemCount - 1)
        ..alignment = alignment;
    });
  }
  ...
  Future<void> _scrollTo({
    required final int index,
    required final double alignment,
    required final Duration duration,
    final Curve curve = Curves.linear,
    required final List<double> opacityAnimationWeights,
  }) async {
    if (_isTransitioning) {
      _stopScroll(canceled: true);
      SchedulerBinding.instance!.addPostFrameCallback((final _) {
        _startScroll(
          index: min(index, widget.itemCount - 1),
          alignment: alignment,
          duration: duration,
          curve: curve,
          opacityAnimationWeights: opacityAnimationWeights,
  ...
  Future<void> _startScroll({
    ...
    final itemPosition =
      ...
      (final itemPosition) => itemPosition.index == index,
    );
  ...
  void _stopScroll({final bool canceled = false}) {
    if (!_isTransitioning) {
      return;
    }
  ...
  Animatable<double> _opacityAnimation(
    final List<double> opacityAnimationWeights,
  ) {
