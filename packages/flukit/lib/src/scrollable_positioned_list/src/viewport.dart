...
class UnboundedRenderViewport extends RenderViewport {
  ...
  UnboundedRenderViewport({
    final AxisDirection axisDirection = AxisDirection.down,
    required final AxisDirection crossAxisDirection,
    required final ViewportOffset offset,
    final double anchor = 0.0,
    final List<RenderSliver>? children,
    final RenderSliver? center,
    final double? cacheExtent,
  })  : _anchor = anchor,
        super(
          axisDirection: axisDirection,
  ...
  @override
  Rect describeSemanticsClip(final RenderSliver? child) {
  ...
  void performLayout() {
    ...
    do {
      ...
      if (correction != 0.0) {
        ...
        final maxScrollOffset = max(min(0, top), bottom);
        final minScrollOffset = min(top, maxScrollOffset);
        if (offset.applyContentDimensions(
          minScrollOffset as double,
          maxScrollOffset as double,
        )) {
          break;
        }
      ...
      () {
        if (count >= _maxLayoutCycles) {
          ...
          throw FlutterError(
            ...
            'RenderViewport render objects, during layout, can retry if either '
            'their slivers or their ViewportOffset decide that the offset '
            'should be corrected to take into account information collected '
            'during that layout.\n'
            'In the case of this RenderViewport object, however, this happened '
            '$count times and still there was no consensus on the scroll '
            'offset. This usually indicates a bug. Specifically, it means '
            'that one of the following three problems is being experienced '
            'by the RenderViewport object:\n'
            ' * One of the RenderSliver children or the ViewportOffset have '
            'a bug such that they always think that they need to correct the '
            'offset regardless.\n'
            ' * Some combination of the RenderSliver children and the '
            'ViewportOffset have a bad interaction such that one applies a '
            'correction then another applies a reverse correction, '
            'leading to an infinite loop of corrections.\n'
            ' * There is a pathological case that would eventually resolve, '
            'but it is so complicated that it cannot be resolved in any '
            'reasonable number of layout passes.',
          );
        }
        return true;
      }(),
  ...
  void updateOutOfBandData(
    ...
    final SliverGeometry childLayoutGeometry,
  ) {
