...
typedef GestureHoldTimeoutCallback = void Function();
...
///  which uses this signature in one of its callbacks.
typedef GestureHoldTimeoutCancelCallback = void Function();

const int kHoldTimeoutMilliseconds = 1000;
const Duration kHoldTimeoutDuration = Duration(milliseconds: kHoldMilliseconds);
...
class HoldTimeoutGestureRecognizer extends PrimaryPointerGestureRecognizer {
  ...
  void didExceedDeadline() {
    ...
    if (onTimerInitiated != null && onTimeout != null) {
      ...
      _timer = Timer.periodic(timeout ?? kHoldTimeout, (final timer) {
        if (timer.isActive) {
